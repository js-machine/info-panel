...
export class FaceDeformer {
  ...
  public load(element: any, points: any, pModel: any, vertices: any): void {
    ...
    this.verticeMap = vertices ? vertices : this.pdmModel.path.vertices;
    ...
    for (let i = 0; i < points.length; i++) {
      if (points[i][0] > this.maxx) {
        this.maxx = points[i][0];
      }
      if (points[i][0] < this.minx) {
        this.minx = points[i][0];
      }
      if (points[i][1] > this.maxy) {
        this.maxy = points[i][1];
      }
      if (points[i][1] < this.miny) {
        this.miny = points[i][1];
      }
    }
    /* tslint:enable */
